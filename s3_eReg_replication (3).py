...
# DBTITLE 1,Cell 6.5 — Discover UUID Folders in S3
# MAGIC %md
# MAGIC ## Cell 6.5 — Discover UUID Folder Mapping
# MAGIC
# MAGIC S3 bucket has **multiple top-level folders**, each containing UUID sub-folders:
# MAGIC ```
# MAGIC s3://bucket/folderA/uuid1/files
# MAGIC s3://bucket/folderA/uuid2/files
# MAGIC s3://bucket/folderB/uuid3/files
# MAGIC ```
# MAGIC
# MAGIC Discovery uses **CommonPrefixes** (Delimiter='/') — returns only "directory" names,
# MAGIC not individual files. This is O(folders), not O(files) — very fast even for large buckets.
# MAGIC
# MAGIC Result: Spark temp view `uuid_folder_map` with (s3_file_id, source_key, parent_folder)
# MAGIC used by `populate_pending_requests` to build correct paths.

# COMMAND ----------

# DBTITLE 1,Discover UUID folders
def discover_uuid_folders(s3_client, bucket: str, prefix: str = "") -> dict:
    """
    Discover UUID folders in the S3 bucket and return a mapping for replication.
    Uses S3 CommonPrefixes with Delimiter='/' — directory-level listing only.
    Does NOT list individual files — very fast.

    Auto-detects two bucket layouts:

      Layout A — flat (1-level):  UUIDs directly under prefix
          s3://bucket/prefix/uuid/files
          s3://bucket/uuid/files           (empty prefix)

      Layout B — nested (2-level): intermediate folders between prefix and UUIDs
          s3://bucket/folderA/uuid/files   (empty prefix)
          s3://bucket/prefix/folderA/uuid/ (with prefix)

    Detection: Step 2 lists sub-folders under each top entry.
      - If sub-folders found   → Layout B (top entries are parent folders)
      - If no sub-folders      → Layout A (top entries ARE the UUIDs)

    Returns: dict  {uuid: {"source_key": "full/s3/key/", "relative_path": "folder/uuid"}}
      source_key    — full S3 prefix used for listing files (passed to boto3)
      relative_path — path relative to search_prefix, used to mirror dest structure
    """
    paginator = s3_client.get_paginator('list_objects_v2')

    # Normalize prefix: ensure trailing slash if non-empty
    search_prefix = f"{prefix}/" if prefix and not prefix.endswith('/') else prefix

    # ── Step 1: List top-level entries under search_prefix ────────────────────
    top_entries = []
    for page in paginator.paginate(Bucket=bucket, Prefix=search_prefix, Delimiter='/'):
        for cp in page.get('CommonPrefixes', []):
            top_entries.append(cp['Prefix'])

    print(f"Found {len(top_entries)} top-level entries under s3://{bucket}/{search_prefix}")
    logger.info(f"Found {len(top_entries)} top-level entries in s3://{bucket}/{search_prefix}")

    # ── Step 2: Try to list sub-folders under each top entry (Layout B check) ─
    uuid_map = {}   # {uuid: {"source_key": ..., "relative_path": ...}}

    for entry in top_entries:
        sub_folders = []
        for page in paginator.paginate(Bucket=bucket, Prefix=entry, Delimiter='/'):
            for cp in page.get('CommonPrefixes', []):
                sub_folders.append(cp['Prefix'])

        if sub_folders:
            # Layout B — entry is a parent folder, sub_folders are UUID dirs
            for sf in sub_folders:
                uuid_value    = sf.rstrip('/').split('/')[-1]
                relative_path = sf[len(search_prefix):].rstrip('/')   # strip search_prefix
                uuid_map[uuid_value] = {"source_key": sf, "relative_path": relative_path}
        else:
            # Layout A — entry itself is a UUID folder (no sub-folders found)
            uuid_value    = entry.rstrip('/').split('/')[-1]
            relative_path = entry[len(search_prefix):].rstrip('/')    # strip search_prefix
            uuid_map[uuid_value] = {"source_key": entry, "relative_path": relative_path}

    layout = "B (nested: folder/uuid/)" if any(
        '/' in v['relative_path'] for v in uuid_map.values()
    ) else "A (flat: uuid/ directly under prefix)"

    print(f"✅ Layout detected  : {layout}")
    print(f"✅ Total UUID folders: {len(uuid_map)}")
    logger.info(f"Discovered {len(uuid_map)} UUID folders — Layout {layout}")
    return uuid_map


# ── Run discovery ──────────────────────────────────────────────────────────────
print("Discovering UUID folders in S3 bucket...")
uuid_folder_mapping = discover_uuid_folders(
    source_client,
    config.source_bucket,
    config.source_prefix   # empty string → scans from bucket root
)

# ── Create Spark temp view for SQL JOINs ──────────────────────────────────────
if uuid_folder_mapping:
    mapping_rows = [
        (uid, v["source_key"], v["relative_path"])
        for uid, v in uuid_folder_mapping.items()
    ]
    mapping_df = spark.createDataFrame(
        mapping_rows, ["s3_file_id", "source_key", "relative_path"]
    )
    mapping_df.createOrReplaceTempView("uuid_folder_map")
    print(f"✅ uuid_folder_map temp view ready ({len(uuid_folder_mapping)} UUIDs)")
    mapping_df.show(5, truncate=False)
else:
    raise RuntimeError(
        f"No UUID folders discovered in s3://{config.source_bucket}/{config.source_prefix} "
        "— check bucket name, prefix, and IAM permissions."
    )
...
def init_tracking_table(config: ReplicationConfig):
    ...
    spark.sql(f"""
        CREATE TABLE IF NOT EXISTS {full_table} (
            ...
            source_key          STRING    NOT NULL,  -- S3 source folder prefix for this UUID
...
def populate_pending_requests(config: ReplicationConfig) -> int:
    ...
    Uses uuid_folder_map (from S3 discovery) to set correct source_key and dest_path.
    INNER JOIN — only inserts UUIDs that actually exist in S3.
    NOT EXISTS guard — fully idempotent, re-runs never create duplicates.
    dest_path mirrors the S3 folder structure: dest_volume_path/parent_folder/uuid
    """
    full_table = f"{config.tracking_schema}.{config.tracking_table}"
    ...
    spark.sql(f"""
        ...
        SELECT
            ...
            '{config.dest_volume_path}/' || m.relative_path                       AS dest_path,
            'PENDING'                                                 AS status,
            0                                                         AS retry_count,
            CURRENT_TIMESTAMP                                         AS created_timestamp

        FROM {config.silver_table} s
        INNER JOIN uuid_folder_map m ON s.s3_file_id = m.s3_file_id
        WHERE NOT EXISTS (
...
def replicate_uuid_folder(config: ReplicationConfig,
    ...
    source_prefix = row['source_key']    # "folder/<uuid>/" (discovered from S3)
